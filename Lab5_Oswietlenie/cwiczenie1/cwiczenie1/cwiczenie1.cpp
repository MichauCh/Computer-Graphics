/* Obrotowe jajko 3d z oswietleniem */ 
#include <windows.h> 
#include <gl/gl.h> 
#include <gl/glut.h> 
#include <math.h> 
...
int x_pos_old = 0, y_pos_old = 0; //poprzednie polozenie myszy 
static int delta_x = 0, delta_y = 0; //zmiana polozenia myszy
const int N = 50; //Liczba punktow na jaka dzielimy kwadrat jednostkowy 
float verLength = 1.0; //Dlugosc boku kwadratu 
float R = 10.0; //Promien sfery obserwatora 
...
void Egg(void) { 
	...
	for (int i = 0; i < N; i++) { 
		for (int j = 0; j < N; j++) { //W jedna strone 
	...
	//W druga strone 
			...
			glNormal3fv(pointsNorms[i + 1][j]);
			glVertex3fv(pointsTab[i + 1][j]); glNormal3fv(pointsNorms[i][j + 1]); glVertex3fv(pointsTab[i][j + 1]); glEnd();
		}
	}
} 
...
// Funkcja okreœlaj¹ca co ma byæ rysowane (zawsze wywo³ywana gdy trzeba // przerysowaæ scenê) 
	void RenderScene(void) { 
		...
		if (status == 2) { // jeœli prawy klawisz myszy wciœniêty 
			...
			if (R <= 6.0) // ograniczenie zblizenia 
				R = 6.0; 
			if (R >= 25.0) // ograniczenie oddalenia 
				R = 25.0; 
		} 
	...
	// Zdefiniowanie po³o¿enia obserwatora 
		Axes(); // Narysowanie osi przy pomocy funkcji zdefiniowanej powy¿ej 
	//Rotacje 
...
//------------------------------------------------------- 
		...
		GLfloat mat_shininess = { 100.0 }; // wspó³czynnik n opisuj¹cy po³ysk powierzchni 
// Definicja Ÿród³a œwiat³a 
//------------------------------------------------------- 
		GLfloat light_position[] { 5.0, 5.0, 10.0, 1.0 }; // po³o¿enie Ÿród³a 
		GLfloat light_ambient[] { 0.1, 0.1, 0.1, 1.0 }; 
		// sk³adowe intensywnoœci œwiecenia Ÿród³a œwiat³a otoczenia // Ia = [Iar,Iag,Iab] 
		...
		GLfloat att_linear = 0.05 ; // sk³adowa liniowa dl dla modelu zmian oœwietlenia w funkcji 
		// odleg³oœci od Ÿród³a
		GLfloat att_quadratic = 0.001; // sk³adowa kwadratowa dq dla modelu zmian oœwietlenia w funkcji 
		// odleg³oœci od Ÿród³a // Ustawienie patrametrów materia³u 
//------------------------------------------------------- 
...
// Funkcja "bada" stan myszy i ustawia wartosci odpowiednich zmiennych globalnych 
	void Mouse(int btn, int state, int x, int y) { 
		...
		// nie zosta³ wciœniêty ¿aden klawisz 
	} 
// Funkcja "monitoruje" polozenie kursora myszy i ustawia wartosci odpowiednich zmiennych globalnych 
	...
	//Dynamiczna alokacja tablicy i wygenerowanie kolorow losowych dla punktow 
		...
		glutCreateWindow("Jajko 3d obrotowe z oswietleniem"); 
		glutDisplayFunc(RenderScene); // Okreœlenie, ¿e funkcja RenderScene bêdzie funkcj¹ zwrotn¹ 
									  // (callback function). Bedzie ona wywo³ywana za ka¿dym razem 
									  // gdy zajdzie potrzba przeryswania okna 
									  // Dla aktualnego okna ustala funkcjê zwrotn¹ odpowiedzialn¹ 
									  // zazmiany rozmiaru okna 
		glutReshapeFunc(ChangeSize); MyInit(); // Funkcja MyInit() (zdefiniowana powy¿ej) wykonuje wszelkie 
		...
		glutMainLoop(); // Funkcja uruchamia szkielet biblioteki GLUT 
						//Zwolnienie pamiêci 
		for (int i = 0; i < N + 1; i++) {
			delete[] pointsTab[i]; delete[] pointsNorms[i]; pointsTab[i] = 0; pointsNorms[i] = 0;
		} delete[] pointsTab; delete[] pointsNorms; pointsTab = 0; pointsNorms = 0;
	}